...
import {
  canDeleteCustomerPayment,
  customerPaymentDeleteMessage,
} from '../customer-payment-delete'
import { PaymentStatusBadge } from '../components/PaymentStatusBadge'
import { useAccountPageChrome } from '../hooks/use-account-page-chrome'

export function CustomerPaymentViewPage() {
  const { id } = useParams()
  ...
  const paymentQuery = useQuery({
    queryKey: queryKeys.account.payments.customer.detail(id!),
  ...
  const statusMutation = useMutation({
    mutationFn: (status: 'cleared' | 'cancelled' | 'voided') =>
      updateCustomerPaymentStatus(id!, status),
    onSuccess: (_, status) => {
      toast.success(
        status === 'cleared'
          ? t('Payment marked as cleared.')
          : status === 'voided'
            ? t('Payment voided. GL and invoice allocations were reversed.')
            : t('Payment cancelled.'),
      )
      void queryClient.invalidateQueries({ queryKey: queryKeys.account.payments.customer.detail(id!) })
      void queryClient.invalidateQueries({ queryKey: queryKeys.account.payments.customer.all() })
    },
    onError: (err) => toast.error(getApiErrorMessage(err, t('Failed to update payment status'))),
  })

  const deleteMutation = useMutation({
    mutationFn: () => deleteCustomerPayment(id!),
    onSuccess: () => {
      toast.success(t('The customer payment has been deleted.'))
      void queryClient.invalidateQueries({ queryKey: ['customer-payments'] })
      navigate(paths.account.customerPayments.index)
    },
    ...
    return (
      <div className="mx-auto max-w-4xl p-6">
        ...
        <Button asChild variant="link" className="mt-2 px-0">
          <Link to={paths.account.customerPayments.index}>{t('Back to list')}</Link>
        </Button>
      </div>
    )
  }
  ...
  const creditApps =
    payment.credit_note_applications ?? payment.creditNoteApplications ?? []
  const totalCreditApplied = creditApps.reduce(
    (sum, app) => sum + Number(app.applied_amount),
    0,
  )
  const showDelete = canDeleteCustomerPayment(
    payment,
    auth.permissions,
    auth.roles,
    auth.user?.type,
  ...
  return (
    <TooltipProvider>
                    ...
                    {payment.allocations.map((row) => (
                      <tr key={row.id} className="border-b">
                        <td className="py-2 font-medium">
                          {row.invoice?.id ? (
                            <Link
                              to={`${paths.sales.invoices}/${row.invoice.id}`}
            ...
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b">
                      <th className="py-2 text-left">{t('Credit note')}</th>
                    ...
                    {creditApps.map((app) => (
                      <tr key={app.id} className="border-b">
                        <td className="py-2 font-medium">
                          {app.credit_note?.id ? (
                            <Link
                              to={paths.account.creditNotes.show(app.credit_note.id)}
                              className="text-primary hover:underline"
                            >
                              {app.credit_note.credit_note_number}
                            </Link>
                          ) : (
                            (app.credit_note?.credit_note_number ?? `#${app.id}`)
                          )}
                        </td>
                        <td className="py-2">
          ...
          <Card>
            ...
            <CardContent className="flex justify-center">
              <img
                ...
                alt={payment.customer?.name ?? t('Payment receipt')}
      ...
      <ConfirmationDialog
        ...
        message={t(
          'This will post a reversing journal entry, undo bank and AR effects, and restore open invoice balances. The original payment record is kept.',
        )}
      ...
      <ConfirmationDialog
        ...
        message={customerPaymentDeleteMessage(payment, t)}
