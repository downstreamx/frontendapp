...
import {
  canDeleteCreditNote,
  creditNoteDeleteMessage,
} from '../credit-note-delete'
import {
  approveCreditNote,
  deleteCreditNote,
  getCreditNote,
...
export function CreditNoteViewPage() {
  ...
  useAccountPageChrome(
    note
      ? `${t('Credit Note')} #${note.credit_note_number ?? note.id}`
      : t('Credit Note Details'),
    t('Credit Notes'),
  )

  const approveMutation = useMutation({
    mutationFn: () => approveCreditNote(id!),
    onSuccess: () => {
      toast.success(t('Credit note approved'))
      void queryClient.invalidateQueries({ queryKey: ['credit-note', id] })
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      void queryClient.invalidateQueries({ queryKey: ['credit-notes'] })
      navigate(paths.account.creditNotes.index)
    },
  ...
  if (noteQuery.error || !note) {
    return (
      <div className="mx-auto max-w-5xl p-6">
        <p className="text-sm text-destructive">{t('Credit note not found.')}</p>
        <Button asChild variant="link" className="mt-2 px-0">
          <Link to={paths.account.creditNotes.index}>{t('Back to list')}</Link>
        </Button>
      </div>
    )
  }

  const showDelete = canDeleteCreditNote(
    note,
    auth.permissions,
    auth.roles,
    auth.user?.type,
  ...
  return (
    <TooltipProvider>
      <div className="mx-auto max-w-5xl space-y-6 p-6">
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              <div>
                ...
                <div className="space-y-1 text-sm">
                  ...
                  {note.customer?.email ? (
                    <div className="text-muted-foreground">{note.customer.email}</div>
                  ) : null}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-1 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span>
                      {note.credit_note_date ? formatDate(String(note.credit_note_date)) : '—'}
                    </span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  {note.sales_return?.return_number ? (
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">{t('Sales Return')}</span>
                      {canViewSalesReturn && note.sales_return.id ? (
                        <Link
                          to={paths.sales.returnShow(note.sales_return.id)}
                          className="text-primary hover:underline"
                        >
                          {note.sales_return.return_number}
                        </Link>
                      ) : (
                        <span>{note.sales_return.return_number}</span>
                      )}
                    </div>
                  ) : null}
                </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t('Credit Note Items')}</CardTitle>
          </CardHeader>
          <CardContent>
                      ...
                      <tr key={application.id}>
                        <td className="px-4 py-4 text-sm">
                          {application.payment?.id ? (
                            <Link
                              to={paths.account.customerPayments.show(application.payment.id)}
                              className="text-primary hover:underline"
                            >
        ...
        <ConfirmationDialog
          ...
          message={creditNoteDeleteMessage(note, t)}
