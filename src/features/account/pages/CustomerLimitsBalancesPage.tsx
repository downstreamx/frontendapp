...
import { fetchCustomerLimitsBalances, type CustomerLimitsBalanceRow } from '../account-customer-limits-api'
...
export function CustomerLimitsBalancesPage() {
  const { t } = useTranslation()
  const [asOfDate, setAsOfDate] = useState(() => todayIso())
  const [applied, setApplied] = useState({ as_of_date: todayIso() })

  useAccountPageChrome(t('Customer Limits & Balances'), t('Customers'))

  const { data, isLoading, isFetching, refetch } = useQuery({
    queryKey: ['account', 'customer-limits-balances', applied],
    queryFn: () => fetchCustomerLimitsBalances({ as_of_date: applied.as_of_date, per_page: '100' }),
  })

  const rows = data?.rows ?? []
  const loading = isLoading || isFetching

  const columns: Column<CustomerLimitsBalanceRow>[] = [
    {
      key: 'company_name',
      header: t('Customer'),
      render: (_, row) => (
        <Link to={paths.account.customerShow(row.id)} className="text-primary hover:underline">
          {row.company_name}
        </Link>
      ),
    },
  ...
  return (
    <ModuleListCard title={t('Customer limits & balances')} isLoading={loading}>
      ...
      {rows.length === 0 && !loading ? (
        <NoRecordsFound
          ...
          description={t('Customer credit limits and balances will appear here.')}
