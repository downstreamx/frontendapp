...
import {
  ...
  fetchCustomerPaymentsIndexMeta,
  listCustomerPaymentsPaginated,
...
function CustomerPaymentRowActions({
  row,
  canView,
  canClear,
  canDelete,
...
export function CustomerPaymentsIndexPage() {
  const { t } = useTranslation()
  ...
  useAccountPageChrome(t('Manage Customer Payments'), t('Customer payments'))
  ...
  const metaQuery = useQuery({
    queryKey: ['customer-payments', 'index-meta'],
    queryFn: fetchCustomerPaymentsIndexMeta,
  })
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['customer-payments', listParams],
    queryFn: () => listCustomerPaymentsPaginated(listParams),
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: (paymentId: number) => deleteCustomerPayment(paymentId),
    onSuccess: () => {
  ...
  const columns: Column<CustomerPaymentRow>[] = [
    {
      key: 'payment_number',
      ...
      render: (value, row) =>
        canView ? (
          <button
            ...
            onClick={() => navigate(paths.account.customerPayments.show(row.id))}
          >
            {String(value ?? `#${row.id}`)}
          </button>
        ) : (
    ...
    },
    {
      ...
      header: t('Actions'),
      render: (_, row) => (
        <CustomerPaymentRowActions
          row={row}
          canView={canView}
          canClear={canClear}
          canDelete={
            canDeletePerm &&
            canDeleteCustomerPayment(row, auth.permissions, auth.roles, auth.user?.type)
          }
          onView={() => navigate(paths.account.customerPayments.show(row.id))}
  ...
  return (
    <TooltipProvider>
      <ModuleListCard
        ...
        canCreate={canCreate}
        onCreateClick={() => navigate(paths.account.customerPayments.create)}
        isLoading={isLoading}
        error={!!error}
        searchToolbar={{
      ...
      <ConfirmationDialog
        ...
        message={
          deleteTarget
            ? customerPaymentDeleteMessage(deleteTarget, t)
