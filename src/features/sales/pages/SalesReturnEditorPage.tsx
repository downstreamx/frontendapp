...
import {
  createSalesReturn,
  fetchSalesReturn,
  fetchSalesReturnCreateMeta,
  updateSalesReturn,
  type EligibleReturnInvoiceItem,
} from '../sales-returns-api'
import { SALES_RETURN_REASON_OPTIONS } from '../sales-return-utils'
...
export function SalesReturnEditorPage() {
  ...
  const pageTitle = isEdit ? t('Edit Sales Return') : t('Create Sales Return')

  usePageChrome({
    pageTitle,
    breadcrumbs: [
      { label: t('Sales'), url: paths.sales.returns },
      { label: t('Sales Returns'), url: paths.sales.returns },
      { label: pageTitle },
    ],
  })

  const metaQuery = useQuery({
    queryKey: ['sales-returns', 'create-meta'],
    queryFn: fetchSalesReturnCreateMeta,
  })
  ...
  useEffect(() => {
    ...
    if (row.status !== 'draft') {
      if (!blockedNonDraftRef.current) {
        ...
        navigate(paths.sales.returns, { replace: true })
      }
      return
    }
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (isEdit && id) return updateSalesReturn(id, body)
      return createSalesReturn(body)
    },
    onSuccess: (saved) => {
      toast.success(
        isEdit
          ? t('The sales return has been updated successfully.')
          : t('The sales return has been created successfully.'),
      )
      void queryClient.invalidateQueries({ queryKey: ['sales-returns'] })
      navigate(paths.sales.returnShow(saved.id))
    },
    onError: (error) => {
  ...
  return (
    ...
    >
        ...
        <CardContent className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={reason} onValueChange={setReason}>
              ...
              <SelectContent>
                {SALES_RETURN_REASON_OPTIONS.map((option) => (
      ...
      <div className="flex justify-end gap-2">
        <Button type="button" variant="outline" onClick={() => navigate(paths.sales.returns)}>
          {t('Cancel')}
        </Button>
