...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-12 w-full items-center justify-between px-4 py-2 text-base",
      formFieldClasses,
      "data-[placeholder]:text-muted-foreground focus:outline-none [&>span]:line-clamp-1",
      className
    )}
    {...props}
  >
...
>(
  ...
  const isControlledSearch = controlledSearch !== undefined && onSearchChange !== undefined
  const search = isControlledSearch ? controlledSearch : internalSearch
  const setSearch = isControlledSearch ? onSearchChange : setInternalSearch

  const filteredChildren = React.useMemo(() => {
    if (!searchable || isControlledSearch || !search) return children

    return React.Children.toArray(children).filter((child) => {
      if (!React.isValidElement<{ children?: React.ReactNode; textValue?: string }>(child)) {
        return true
      }
      const textValue = child.props.textValue
  ...
  }, [children, isControlledSearch, search, searchable])
