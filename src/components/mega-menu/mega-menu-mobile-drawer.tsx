...
function MegaMenuMobileNavTree({
  ...
  return (
    <ul className={cn('space-y-1', depth > 0 && 'ms-3 border-s ps-3')}>
      {items.map((item) => {
          ...
          return (
            <li key={item.title}>
              <Collapsible defaultOpen={shouldBeActive} className="group/collapsible">
                <CollapsibleTrigger className="flex w-full items-center gap-2 rounded-md px-2 py-2 text-sm hover:bg-accent">
        ...
        return (
          <li key={item.title}>
            {item.href ? (
              ...
              <span className="flex items-center gap-2 px-2 py-2 text-sm text-muted-foreground">
                <MenuLucideIcon item={item} />
                {item.title}
              </span>
            )}
